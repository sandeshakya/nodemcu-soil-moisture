...
void setup()
{
  ...
  {
    serializeJson(CreatePublishObj(sensors_id[i], now, sensors_data[i]), jsonObj);
    topic = SENSOR_PUBLISH_TOPIC + String(sensors_id[i]);
    topic.toCharArray(topic_char, topic.length());
    client.publish(topic_char, jsonObj);
  }
  serializeJson(CreatePublishObj(1, now, battery_level), jsonObj);
  ...
  Serial.println(sensor_1_threshold);
  Serial.println(sensor_2_threshold);
